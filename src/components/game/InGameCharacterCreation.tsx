...
import { DIVINES } from '../../game/data/Divines';
import { AlignmentSystem } from '../../game/utils/Alignment';

interface InGameCharacterCreationProps {
  onCreate: (data: CharacterCreationData) => void;
  onClose: () => void;
}

const InGameCharacterCreation: React.FC<InGameCharacterCreationProps> = ({ onCreate, onClose }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-game-panel border-2 border-game-border rounded-lg p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-4xl font-bold text-game-text font-game">Create Your Character</h1>
          <button
            onClick={onClose}
            className="text-game-text hover:text-game-health text-2xl font-bold"
          >
            ×
          </button>
        </div>
...
export default InGameCharacterCreation;
